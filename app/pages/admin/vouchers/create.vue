...
const { createVoucher } = useVouchersApi();
...
const generateCode = () => {
  form.code = `FOOD${Math.random().toString(36).slice(2, 7).toUpperCase()}`;
};
          ...
          <div class="grid gap-4 mt-4 md:grid-cols-2">
            <div>
              ...
              <div class="mt-2 flex gap-2">
                <InputText v-model="form.code" class="flex-1" />
                <button
                  type="button"
                  class="rounded-full border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 transition hover:border-slate-300 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:border-slate-500"
                  @click="generateCode"
                >
                  Generate
                </button>
