<template>
  ...
  >
        ...
        <div class="mt-[14px] mount-3" aria-live="polite">
          ...
          <div class="mt-[10px] grid grid-cols-3 gap-2">
            ...
            <article
              ...
              "
            >
              <strong class="text-[1.1rem]">{{
                freeshipsAvailable ? "0đ" : "N/A"
              }}</strong>
              <span class="text-[0.76rem] opacity-[0.86]"
            ...
            >
              <strong class="text-[1.1rem]">{{
                maxDiscountPercent > 0 ? `${maxDiscountPercent}%` : "--"
              }}</strong>
    ...
    <main class="w-[min(1120px,92vw)] mx-auto py-6 pb-14 grid gap-7">
      <!-- Featured -->
      <section v-if="isLoading || featuredCoupons.length > 0">
        ...
        <VoucherFeaturedSkeleton v-if="isLoading" />
        <div
          v-else
        ...
        >
          ...
          >
            ...
            <!-- Status overlay (Safety for Featured) -->
            <div
              v-if="coupon.status !== 'active'"
              class="absolute inset-0 bg-white/40 grid place-items-center z-10"
            >
              <span
      ...
      <section>
        ...
        <VoucherCardSkeleton v-if="isLoading" />
...
import { useVouchersPage } from "~/composables/voucher/useVouchersPage";

useHead({
  title: "Ví Voucher - SmartFood",
  meta: [
    {
      name: "description",
      content: "Kho mã giảm giá SmartFood - Săn deal hot, tiết kiệm mỗi ngày.",
    },
  ],
});
